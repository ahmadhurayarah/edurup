...
const CourseBanner = ({ cityName = "Bangalore" }: CourseBannerProps) => {
  ...
  return (
    <section
      className={`${banner.backgroundColor} ${banner.textColor} py-12 px-4 sm:px-16`}
      style={{
        ...
        <div className="flex-1 flex flex-col gap-4 sm:gap-6">
          ...
          <p
            className={`text-base sm:text-lg ${
              banner.textColor === "black" ? "text-black" : "text-white"
            }`}
          >
            {course.description}
          </p>
          ...
          {/* Features */}
          <div
            className={`flex flex-col sm:flex-row flex-wrap gap-4 sm:gap-6 mt-4 ${
              banner.textColor === "black" ? "text-black" : "text-white"
            }`}
          >
            {course.features.map((feature, idx) => (
              <div
                ...
                className={`flex flex-col items-start ${
                  banner.textColor === "black" ? "bg-white" : "bg-black/20"
            ...
            <Button
              className={`${
                banner.textColor === "black"
                  ...
                  : "bg-white text-black hover:bg-gray-200"
              } w-full sm:w-auto py-3 sm:py-2 transition text-sm sm:text-base`}
            >
              Download Curriculum
            </Button>
          </div>
        ...
        <div className="flex-1 w-full lg:max-w-lg bg-white text-black rounded-2xl shadow-2xl p-6 sm:p-8 relative">
          ...
          <div className="mb-5">
            ...
            <p className="text-xs sm:text-sm text-gray-600 mt-2">
              EMI starts from ₹{course.price.emi.toLocaleString("en-IN")}
              /month
            </p>
          </div>
